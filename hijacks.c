...
#ifdef CONFIG_X86_64
	if (!rtb_insn_rip_relative(src_insn))
		return;
...
static inline pte_t *rtb_lookup_address(unsigned long address, unsigned int *level)
{
...
#else
#define rtb_lookup_address(address, level) lookup_address(address, level);
#endif
...
static inline void set_addr_rw(unsigned long addr, bool *flag) {
	...
	*flag = true;
	...
	pte = rtb_lookup_address(addr, &level);
