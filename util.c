...
void hook_sys_write ( void *new)
{
	...
	DEBUG_HOOK("Hooking function sys_write");

    o_cr0 = disable_wp();
    sys_call_table[__NR_write] = (unsigned long)new;
    restore_wp(o_cr0);
...
void unhook_sys_write(void *orig)
{
	...
	DEBUG_HOOK("unHooking function sys_write");

    o_cr0 = disable_wp();
    sys_call_table[__NR_write] = (unsigned long)orig;
    restore_wp(o_cr0);
